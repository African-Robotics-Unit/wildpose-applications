...
FPS = 3
OUTPUT_VIDEO_PATH = 'seg_lion_sleep3.avi'
...
gt_json = json.load(open('data/lion_sleep3/train.json', 'r'))
img_dict = {}
...
# for key in img_dict:
for key in tqdm(range(len(images))):
    # if key > 100:
    #    break
    # if key < 7:
    #    continue
    img_name = images[key]['file_name']
    ...
    masks_list = []
    if key in img_dict:
        # cv2.putText(img, 'Frame Number: {0}'.format(key), (20, 40), font, 1, (255,255,255), thickness, cv2.LINE_AA)
        for obj_idx in range(len(img_dict[key])):
            ...
            cv2.putText(
                ...
                thickness,
                cv2.LINE_AA)
    np.save('./masks_lion/{0}.npy'.format(key), masks_list)

    np.save('./masks_lion/{0}_obj_ids.npy'.format(key), np.array(objs))
