...
def visualize_open3d(pcd_with_rgb, obj_pos_colors, obj_dict, cam_plotter=None, obj_points=[]):
    ...
    '''
    axes = o3d.geometry.TriangleMesh.create_coordinate_frame(size=1.0, origin=np.array([1, 0., 20.]))
    ...
    if len(obj_points) > 0:
        ...
        for mesh_idx in range(len(obj_points)):
            ...
            if obj_id == 1:
                ...
                mask = (all_pos[:,2]> 18.6) * (all_pos[:,2]< 19)*  (all_pos[:,0]> -1.7) * (all_pos[:,0]< 0.0)#obj_pos[:,0].min()) * (all_pos[:,0] < obj_pos[:,0].max())
                
                obj_pos = all_pos[mask]
                mask2 = (obj_pos[:,0] >-1.2) * (obj_pos[:,0] <-0.5) * (obj_pos[:,1] > 0.1)
                mask2 = mask2 == False
                obj_pos = obj_pos[mask2]
                mask3 = (obj_pos[:,0] >-0.1) * (obj_pos[:,1] > 0.1)
                mask3 = mask3 == False
                
                obj_pos = obj_pos[mask3]
                mask4 =  (obj_pos[:,1] > 0.13)
                mask4 = mask4 == False
                obj_pos = obj_pos[mask4]
                #pdb.set_trace()
...
def construct_cam_plotter(scale=0.5, translation=(0,0,8)):
...
file_path_lidar = 'various_pcds/martial_eagle/martial_eagle/lidar/'
file_path_rgb = 'various_pcds/martial_eagle/martial_eagle/sync_rgb/'
file_path_mask = './masks_martial_eagle/'
...
FPS = 4
OUTPUT_VIDEO_PATH = 'output_martial_eagle_.avi' 
...
img_dict = get_2D_gt('martial_eagle/train.json')
