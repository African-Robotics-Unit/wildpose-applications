...
def visualize_open3d(pcd_with_rgb, obj_pos_colors, obj_dict, cam_plotter=None, obj_points=[]):
    ...
    rgb_values = []
    max_depth = 100.0
    depth_values = np.clip(depth_values, 0, 100)
    ...
    '''
    axes = o3d.geometry.TriangleMesh.create_coordinate_frame(size=1.0, origin=np.array([1, -0.5, 38.]))
    ...
    if len(obj_points) > 0:
        ...
        for mesh_idx in range(len(obj_points)):
            ...
            if obj_id == 1:
                ...
                mask = (all_pos[:,2]> 33.6) * (all_pos[:,2]< 34.5) * (all_pos[:,0]> -0.3)
                
                obj_pos = all_pos[mask]
                mask2 = (obj_pos[:,0]> 0.5) * (obj_pos[:,1] > 0.0)
                mask2 = mask2 == False
...
def construct_cam_plotter(scale=0.5, translation=(0,0,22)):
...
file_path_lidar = 'various_pcds/secretarybird/secretarybird/lidar/'
file_path_rgb = 'various_pcds/secretarybird/secretarybird/sync_rgb/'
file_path_mask = './masks_secretarybird/'
...
FPS = 4
OUTPUT_VIDEO_PATH = 'output_secretarybird_eagle_.avi' 
...
img_dict = get_2D_gt('secretarybird/train.json')
