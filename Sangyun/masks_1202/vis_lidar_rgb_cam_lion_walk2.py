...
def visualize_open3d(pcd_with_rgb, obj_pos_colors, obj_dict, cam_plotter=None, obj_points=[]):
    ...
    '''
    axes = o3d.geometry.TriangleMesh.create_coordinate_frame(size=1.0, origin=np.array([2, 0., 40.]))
    ...
    if len(obj_points) > 0:
        ...
        for mesh_idx in range(len(obj_points)):
            ...
            if obj_id == 1:
                ...
                mask = (all_pos[:,2]> 44) * (all_pos[:,2]< 45)*  (all_pos[:,0]> -1.2) * (all_pos[:,0]< 0.3)#obj_pos[:,0].min()) * (all_pos[:,0] < obj_pos[:,0].max())
            ...
            for p_idx in range(len(obj_pos)):
                ...
                mesh_on_surface = o3d.geometry.TriangleMesh.create_sphere(radius=0.04)    
...
file_path_lidar = 'various_pcds/lion_walk/lion_walk/lidar/'
file_path_rgb = 'various_pcds/lion_walk/lion_walk/sync_rgb/'
file_path_mask = './masks_lion_walk/'
...
FPS = 4
OUTPUT_VIDEO_PATH = 'output_lion_walk_.avi' 
...
img_dict = get_2D_gt('lion_walk/train.json')
