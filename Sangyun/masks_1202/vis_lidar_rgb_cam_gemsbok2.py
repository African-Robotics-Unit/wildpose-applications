...
def visualize_open3d(pcd_with_rgb, obj_pos_colors, obj_dict, cam_plotter=None, obj_points=[]):
    ...
    '''
    axes = o3d.geometry.TriangleMesh.create_coordinate_frame(size=1.0, origin=np.array([4, 0., 55.]))
    ...
    if len(obj_points) > 0:
        ...
        for mesh_idx in range(len(obj_points)):
            ...
            if obj_id == 1:
                ...
                mask = (all_pos[:,2]> 55) * (all_pos[:,2]< 56.5)*  (all_pos[:,0]> -0.95) * (all_pos[:,0]< 0.8)#obj_pos[:,0].min()) * (all_pos[:,0] < obj_pos[:,0].max())
            ...
            if obj_id == 2:
                #pdb.set_trace()
                mask = (all_pos[:,2]> 60) * (all_pos[:,2]< 62.5)*  (all_pos[:,0]> 0.6) * (all_pos[:,1]< 0.5)#obj_pos[:,0].min()) * (all_pos[:,0] < obj_pos[:,0].max())
                
                obj_pos = all_pos[mask]
            if obj_id == 3:
                #pdb.set_trace()
                mask = (all_pos[:,2]> 59) * (all_pos[:,2]< 61)*  (all_pos[:,0]< -0.8)# * (all_pos[:,1]< 0.5)#obj_pos[:,0].min()) * (all_pos[:,0] < obj_pos[:,0].max())
                
                obj_pos = all_pos[mask]
            for p_idx in range(len(obj_pos)):

                mesh_on_surface = o3d.geometry.TriangleMesh.create_sphere(radius=0.02)    
                if obj_id > 1:
                    mesh_on_surface = o3d.geometry.TriangleMesh.create_sphere(radius=0.03)    
...
def construct_cam_plotter(scale=0.5, translation=(0,0,28)):
...
file_path_lidar = 'various_pcds/gemsbok/gemsbok/lidar/'
file_path_rgb = 'various_pcds/gemsbok/gemsbok/sync_rgb/'
file_path_mask = './masks_gemsbok/'
...
img_dict = get_2D_gt('gemsbok/train.json')
